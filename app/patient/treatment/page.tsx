        ...
        {!treatment ? (
          ...
          <>
            ...
            <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm space-y-6">
              ...
              {[1, 2].map((doseNum) => (
                <div key={doseNum} className="p-4 bg-gray-50 border border-gray-200 rounded-lg flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div>
                    <h4 className="font-semibold text-gray-900">Dose {doseNum}</h4>
                    <p className="text-sm text-gray-500">
                      Status: <span className="font-medium uppercase text-xs ml-1">
                        {getDoseStatus(doseNum) === 'taken' ? '✅ Taken' : getDoseStatus(doseNum) === 'missed' ? '❌ Missed' : 'Pending'}
                      </span>
                    </p>
                  </div>
                  <div className="flex gap-2 w-full sm:w-auto">
                    <button
                      onClick={() => handleLogDose(doseNum, "taken")}
                      className={`flex-1 sm:flex-none px-4 py-2 text-sm font-medium rounded-md transition ${
                        getDoseStatus(doseNum) === "taken" 
                    ...
                    <button
                      onClick={() => handleLogDose(doseNum, "missed")}
                      className={`flex-1 sm:flex-none px-4 py-2 text-sm font-medium rounded-md transition ${
                        getDoseStatus(doseNum) === "missed" 
